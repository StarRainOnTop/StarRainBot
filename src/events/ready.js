...
export default {
  ...
  async execute(client) {
    try {
      ...
      // 🚀 註冊指令：透過帶入 guildId 進行指定伺服器註冊（分身帳號可秒速看到指令）
      // 如果將來要上線給全部伺服器，只需把下面的 guildId 刪除即可變回全域註冊
      await registerCommands(client, { 
        clientId: client.user.id,
        guildId: "783858618386219059" // 👈 請把這裡換成你實際的測試伺服器 ID
      });
